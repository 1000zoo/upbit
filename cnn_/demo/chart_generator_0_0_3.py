...
def get_ohlcv_order(ticker="KRW-BTC", count=TCOUNT):
    ...
    for il in interval_list:
        interval = M + str(il)
        for to in generate_to_order(il):
            if int(to) > int(now_toformat()):
                break
            print(to)
            yield pu.get_ohlcv(ticker, count=count, interval=interval, to=to)
...
def generate_images(count=1000, path=TRAIN, ticker="KRW-BTC", shuffle=True):
    ...
    ## main loop
    for candle in get(ticker=ticker):
        ## 종료 조건문
        ...
        mpf.plot(candle[:TCOUNT - 1], type='candle', volume=True, style=s, savefig="train.jpg")
        mpf.plot(candle[:TCOUNT], type='candle', volume=True, style=s, savefig="full.jpg")
        ...
        ## p2 와 p3 의 내분 값
        pinc = 0.75*p2 + 0.25*p3
        pdec = 0.25*p2 + 0.75*p3
        moveto = path
...
if __name__ == "__main__":
    ...
    generate_images(1000, TRAIN, shuffle=False)
    print("generate validation data")
    generate_images(200, VAL, shuffle=False)
    print("generate test data")
    generate_images(100, TEST, shuffle=False)
